/** \file ThermalConvectionSimulation.cpp
...
// Class include
//
#include "Simulations/Implementations/ThermalConvectionSimulation.hpp"
...
namespace EPMDynamo {
   ...
   ThermalConvectionSimulation::ThermalConvectionSimulation()
   ...
   void ThermalConvectionSimulation::initEquations()
   {
   ...
   void ThermalConvectionSimulation::configureTransforms()
   {
      //
   ...
   void ThermalConvectionSimulation::updateEquationsRTP(const int step)
   {
   ...
   void ThermalConvectionSimulation::updateEquationsRHS()
   {
   ...
   void ThermalConvectionSimulation::transformEquationsRHS(const int step)
   {
      // Update RHS of the transport equation
   ...
   void ThermalConvectionSimulation::addExternalInfluence()
   {
      // Add Thermal source term
      this->mTransport.addSourceTerm();
   }

   void ThermalConvectionSimulation::timestepEquations()
   {
   ...
   void ThermalConvectionSimulation::initFields()
   {
      // Create a state file reader for the initial state
      EPMSHARED_PTR<StateFileReader<ThermalConvectionTraits> > pInState(new StateFileReader<ThermalConvectionTraits>(this->mCodC, this->mVelV,  "_initial"));

      // Read in initial state
      this->mIOSys.useInitialState(pInState, this->mSimControl.tsParams());

      // Create a source file reader for the codensity source
      EPMSHARED_PTR<SourceFileReader<ThermalConvectionTraits> > pSource(new SourceFileReader<ThermalConvectionTraits>(this->mCodC));
   ...
   void ThermalConvectionSimulation::addHDF5Output()
   {
      EPMSHARED_PTR<StateFileWriter<ThermalConvectionTraits> >  pOutState(new StateFileWriter<ThermalConvectionTraits>(this->mCodC, this->mVelV, this->mEqParams, this->mSimControl.tsParams()));

      this->mIOSys.addHDF5Writer(pOutState);
   }

   void ThermalConvectionSimulation::addASCIIOutput()
   {
      ...
      EPMSHARED_PTR<EnergyFile<ThermalConvectionTraits::CodType> > pCodEnergy(new EnergyFile<ThermalConvectionTraits::CodType>(this->mCodC, "cod", this->mSimControl.tsParams()));
      // Create a energy ASCII diagnostic file for the velocity field
      EPMSHARED_PTR<EnergyFile<ThermalConvectionTraits::VelType> > pVelEnergy(new EnergyFile<ThermalConvectionTraits::VelType>(this->mVelV, "vel", this->mSimControl.tsParams()));
      ...
      EPMSHARED_PTR<SpectrumFile<ThermalConvectionTraits::CodType> > pCodSpectrum(new SpectrumFile<ThermalConvectionTraits::CodType>(this->mCodC, "cod"));
      // Create a energy spectrum ASCII diagnostic file for the velocity field
      EPMSHARED_PTR<SpectrumFile<ThermalConvectionTraits::VelType> > pVelSpectrum(new SpectrumFile<ThermalConvectionTraits::VelType>(this->mVelV, "vel"));
