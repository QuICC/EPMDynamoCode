...
namespace EPMDynamo {
   ...
   template <typename TSimType> MagnetoConvectionSimulation<TSimType>::MagnetoConvectionSimulation()
      : mCFLFactor(1.0), mMagB(this->mpSTrunc, this->mTransform), mVelV(this->mpSTrunc, this->mTransform), mInduction(mMagB, mVelV, this->mTransform, this->mSimControl.tsParams()), mNavierStokes(mVelV, mMagB, this->mTransform, this->mSimControl.tsParams(), this->mEqParams)
   {
   ...
   template <typename TSimType> void MagnetoConvectionSimulation<TSimType>::configureTransforms()
   {
      // Set basic configuration for Grouped simple
         ...
         this->configureWSHManipulator(6, 12, false);
      #else
         // Set number of transform steps
      ...
      // Set basic configuration for Grouped massive
         ...
         // Initialise the data buffers for communication
         this->configureSHManipulator(6, 12, false);
