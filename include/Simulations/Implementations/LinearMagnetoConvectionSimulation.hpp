/** \file LinearMagnetoConvectionSimulation.hpp
 *  \brief Implementation of the linear magneto-convection simulation
 */

#ifndef LINEARMAGNETOCONVECTIONSIMULATION_HPP
#define LINEARMAGNETOCONVECTIONSIMULATION_HPP
...
#include "Simulations/Traits/LinearMagnetoConvectionTraits.hpp"
...
#include "Equations/NavierStokes/NavierStokesLinMHD.hpp"
...
namespace EPMDynamo {
    ...
    */
   template <typename TSimType> class LinearMagnetoConvectionSimulation: public SimulationBase<TSimType>
   {
      public:
         /**
          * @brief Simple empty destructor
          */
         virtual ~LinearMagnetoConvectionSimulation() {};

      protected:
         /**
          * @brief Constructor
          */
         LinearMagnetoConvectionSimulation();
      ...
      private:
          ...
          */
         typename LinearMagnetoConvectionTraits<TSimType>::CodType   mCodC;

         /**
          * @brief Magnetic field
          */
         typename LinearMagnetoConvectionTraits<TSimType>::MagType   mMagB;

         /**
          * @brief Velocity field
          */
         typename LinearMagnetoConvectionTraits<TSimType>::VelType   mVelV;
          ...
          */
         NavierStokesLinMHD<TSimType, LinearMagnetoConvectionTraits>    mNavierStokes;
   };

   template <typename TSimType> LinearMagnetoConvectionSimulation<TSimType>::LinearMagnetoConvectionSimulation()
   ...
   template <typename TSimType> void LinearMagnetoConvectionSimulation<TSimType>::initEquations()
   {
   ...
   template <typename TSimType> void LinearMagnetoConvectionSimulation<TSimType>::updateEquationsRTP(const int step)
   {
   ...
   template <typename TSimType> void LinearMagnetoConvectionSimulation<TSimType>::updateEquationsRHS()
   {
   ...
   template <typename TSimType> void LinearMagnetoConvectionSimulation<TSimType>::transformEquationsRHS(const int step)
   {
   ...
   template <typename TSimType> void LinearMagnetoConvectionSimulation<TSimType>::addExternalInfluence()
   {
      // Add Thermal source term
      this->mTransport.addSourceTerm();
   }

   template <typename TSimType> void LinearMagnetoConvectionSimulation<TSimType>::timestepEquations()
   {
   ...
   template <typename TSimType> void LinearMagnetoConvectionSimulation<TSimType>::configureTransforms()
   {
      //
   ...
   template <typename TSimType> void LinearMagnetoConvectionSimulation<TSimType>::initFields()
   {
      // Read in initial state
      EPMSHARED_PTR<StateFileReader<TSimType, LinearMagnetoConvectionTraits> > pInState(new StateFileReader<TSimType, LinearMagnetoConvectionTraits>(this->mCodC, this->mMagB, this->mVelV,  "_initial"));

      this->mIOSys.useInitialState(pInState, this->mSimControl.tsParams());

      // Create a source file reader for the codensity source
      EPMSHARED_PTR<SourceFileReader<TSimType, LinearMagnetoConvectionTraits> > pSource(new SourceFileReader<TSimType, LinearMagnetoConvectionTraits>(this->mCodC));
      ...
      EPMSHARED_PTR<ImposedFieldReader<TSimType, LinearMagnetoConvectionTraits> > pIField(new ImposedFieldReader<TSimType, LinearMagnetoConvectionTraits>(this->mMagB));
   ...
   template <typename TSimType> void LinearMagnetoConvectionSimulation<TSimType>::addHDF5Output()
   {
      EPMSHARED_PTR<StateFileWriter<TSimType, LinearMagnetoConvectionTraits> >  pOutState(new StateFileWriter<TSimType, LinearMagnetoConvectionTraits>(this->mCodC, this->mMagB, this->mVelV, this->mEqParams, this->mSimControl.tsParams()));

      this->mIOSys.addHDF5Writer(pOutState);
   }

   template <typename TSimType> void LinearMagnetoConvectionSimulation<TSimType>::addASCIIOutput()
   {
      // Create a energy ASCII diagnostic file for the codensity scalar
      EPMSHARED_PTR<EnergyFile<TSimType, typename LinearMagnetoConvectionTraits<TSimType>::CodType> > pCodEnergy(new EnergyFile<TSimType, typename LinearMagnetoConvectionTraits<TSimType>::CodType>(this->mCodC, "cod", this->mSimControl.tsParams()));
      // Create a energy ASCII diagnostic file for the magnetic field
      EPMSHARED_PTR<EnergyFile<TSimType, typename LinearMagnetoConvectionTraits<TSimType>::MagType> > pMagEnergy(new EnergyFile<TSimType, typename LinearMagnetoConvectionTraits<TSimType>::MagType>(this->mMagB, "mag", this->mSimControl.tsParams()));
      // Create a energy ASCII diagnostic file for the velocity field
      EPMSHARED_PTR<EnergyFile<TSimType, typename LinearMagnetoConvectionTraits<TSimType>::VelType> > pVelEnergy(new EnergyFile<TSimType, typename LinearMagnetoConvectionTraits<TSimType>::VelType>(this->mVelV, "vel", this->mSimControl.tsParams()));
      ...
      EPMSHARED_PTR<SpectrumFile<TSimType, typename LinearMagnetoConvectionTraits<TSimType>::CodType> > pCodSpectrum(new SpectrumFile<TSimType, typename LinearMagnetoConvectionTraits<TSimType>::CodType>(this->mCodC, "cod"));
      // Create a energy spectrum ASCII diagnostic file for the magnetic field
      EPMSHARED_PTR<SpectrumFile<TSimType, typename LinearMagnetoConvectionTraits<TSimType>::MagType> > pMagSpectrum(new SpectrumFile<TSimType, typename LinearMagnetoConvectionTraits<TSimType>::MagType>(this->mMagB, "mag"));
      // Create a energy spectrum ASCII diagnostic file for the velocity field
      EPMSHARED_PTR<SpectrumFile<TSimType, typename LinearMagnetoConvectionTraits<TSimType>::VelType> > pVelSpectrum(new SpectrumFile<TSimType, typename LinearMagnetoConvectionTraits<TSimType>::VelType>(this->mVelV, "vel"));
...
#endif // LINEARMAGNETOCONVECTIONSIMULATION_HPP
