/** \file NavierStokesNLPrecessionFrame.hpp
...
#ifndef NAVIERSTOKESNLPRECESSIONFRAME_HPP
#define NAVIERSTOKESNLPRECESSIONFRAME_HPP
...
namespace EPMDynamo {
    ...
    */
   template <typename TSimTraits> class NavierStokesNLPrecessionFrame : public NavierStokesDiffusion<TSimTraits>
   {
      public:
          ...
          */
         NavierStokesNLPrecessionFrame(typename TSimTraits::VelType &rV, TransformType &transform, TimestepParameters &tsteps, EquationParametersType &params);

         /**
          * @brief Simple empty destructor
          */
         virtual ~NavierStokesNLPrecessionFrame() {};
   ...
   template <typename TSimTraits> NavierStokesNLPrecessionFrame<TSimTraits>::NavierStokesNLPrecessionFrame(typename TSimTraits::VelType &rV, typename NavierStokesNLPrecessionFrame<TSimTraits>::TransformType &transform, TimestepParameters &tsteps,  typename NavierStokesNLPrecessionFrame<TSimTraits>::EquationParametersType &params)
      : NavierStokesDiffusion<TSimTraits>(rV, transform, tsteps, params), mOmega(0.0), mCosAlpha(0.0), mSinAlpha(0.0)
   {
      // Setup precession forcing
      this->setupPrecession();
   }

   template <typename TSimTraits> void NavierStokesNLPrecessionFrame<TSimTraits>::setupPrecession()
   {
   ...
   template <typename TSimTraits> void NavierStokesNLPrecessionFrame<TSimTraits>::updateRTP(const int step)
   {
   ...
   template <typename TSimTraits> void NavierStokesNLPrecessionFrame<TSimTraits>::updateRHS()
   {
      // Compute \f$u\times (\nabla \times u) \f$
...
#endif // NAVIERSTOKESNLPRECESSIONFRAME_HPP
